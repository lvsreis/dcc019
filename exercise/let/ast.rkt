...
Exp ::= INT | VAR
     |  (ZERO? Exp) | (DIFF Exp Exp)
     |  (IF Exp Exp Exp) | (LET VAR Exp Exp)
     | (ADD Exp Exp) | (MULT Exp Exp) | (DIV Exp Exp)   ; rules for the extension
...
; Structures for the add, mult and div extension
(struct add-ast exp-node (lexp rexp) #:transparent)
(struct mult-ast exp-node (lexp rexp) #:transparent)
(struct div-ast exp-node (lexp rexp) #:transparent)
